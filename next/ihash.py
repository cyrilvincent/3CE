from PIL import Image
import imagehash
import numpy as np
import os

size = os.stat('../old/images/0/ski.jpg')[6]
im:Image = Image.open('../old/images/0/ski.jpg')
ah = imagehash.average_hash(im) #8x8
dh = imagehash.dhash(im) #8x8
ph = imagehash.phash(im) #8x8
wh = imagehash.whash(im) #Haar 8x8
wdh = imagehash.whash(im, mode="db4") #Daubechies 14x14
ch = imagehash.colorhash(im) #14*3
print(ah,ph,dh,wh,wdh,ch, size)
size2 = os.stat('../old/images/0/ski2.jpg')[6]
im2 = Image.open('../old/images/0/ski2.jpg')
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
ch2 = imagehash.colorhash(im2)
print(ah2,ph2,dh2,wh2,wdh2,ch2, size2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)
im2 = Image.open('../old/images/0/ski3.jpg')
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
ch2 = imagehash.colorhash(im2)
print(ah2,ph2,dh2,wh2,wdh2,ch2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)
im2 = Image.open('../old/images/1/mug.jpg')
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
ch2 = imagehash.colorhash(im2)
print(ah2,ph2,dh2,wh2,wdh2,ch2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)
im = Image.open('../old/images/0/ski2.jpg')
ah = imagehash.average_hash(im)
dh = imagehash.dhash(im)
ph = imagehash.phash(im)
wh = imagehash.whash(im)
wdh = imagehash.whash(im, mode="db4")
im2 = Image.open('../old/images/0/ski3.jpg')
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
print(ah - ah2, ph - ph2, dh - dh2, wh - wh2, wdh - wdh2)
im = Image.open('../old/images/1/mug.jpg')
ah = imagehash.average_hash(im)
dh = imagehash.dhash(im)
ph = imagehash.phash(im)
wh = imagehash.whash(im)
wdh2 = imagehash.whash(im2, mode="db4")
print(ah - ah2, ph - ph2, dh - dh2, wh - wh2, wdh - wdh2)

def alpharemover(image):
    if image.mode != 'RGBA':
        return image
    canvas = Image.new('RGBA', image.size, (255,255,255,255))
    canvas.paste(image, mask=image)
    return canvas.convert('RGB')

def ztransform(image):
    image = alpharemover(image)
    image = image.convert("L").resize((8, 8), Image.ANTIALIAS)
    data = image.getdata()
    quantiles = np.arange(100)
    quantiles_values = np.percentile(data, quantiles)
    zdata = (np.interp(data, quantiles_values, quantiles) / 100 * 255).astype(np.uint8)
    image.putdata(zdata)
    return image
print("ztransform")
im = Image.open('../old/images/0/ski.jpg')
im = ztransform(im)
ah = imagehash.average_hash(im)
dh = imagehash.dhash(im)
ph = imagehash.phash(im)
wh = imagehash.whash(im)
wdh = imagehash.whash(im, mode="db4")
print(ah,ph,dh,wh,wdh)
im2 = Image.open('../old/images/0/ski2.jpg')
im2 = ztransform(im2)
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
print(ah2,ph2,dh2,wh2,wdh2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)
im2 = Image.open('../old/images/0/ski3.jpg')
im2 = ztransform(im2)
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
print(ah2,ph2,dh2,wh2,wdh2,ch2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)
im2 = Image.open('../old/images/1/mug.jpg')
im2 = ztransform(im2)
ah2 = imagehash.average_hash(im2)
ph2 = imagehash.phash(im2)
dh2 = imagehash.dhash(im2)
wh2 = imagehash.whash(im2)
wdh2 = imagehash.whash(im2, mode="db4")
print(ah2,ph2,dh2,wh2,wdh2,ch2)
print((ah - ah2) / len(ah.hash)**2, (ph - ph2) / len(ph.hash)**2, (dh - dh2) / len(dh.hash)**2, (wh - wh2) / len(wh.hash)**2, (wdh - wdh2) / len(wdh.hash)**2, ch - ch, size - size2)